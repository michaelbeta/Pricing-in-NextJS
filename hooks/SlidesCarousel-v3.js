...
const SlidesCarousel = (props) => {
    ...
    useEffect(() => {
        ...
        props.slides.forEach((slide) => {
            const slideobject = {
                class: 'slider-single-v3 proactivede-v3',
                element: slide,
            };
    ...
    const slideRight = () => {
        ...
        const activeClass = 'slider-single-v3 active-v3';
        const slide = [...slides];
        if (slideTotal > 1) {
            ...
            slide.forEach((slid, index) => {
                if (slid.class.includes('preactivede-v3')) {
                    slid.class = 'slider-single-v3 proactivede-v3';
                }
                if (slid.class.includes('preactive-v3')) {
                    slid.class = 'slider-single-v3 preactivede-v3';
                }
            });

            preactiveSlide.class = 'slider-single-v3 preactive-v3';
            activeSlide.class = activeClass;
            proactiveSlide.class = 'slider-single-v3 proactive-v3';
            setSlides(slide);
            setSlideCurrent(slideCurrentLoc);

            if (document.getElementsByClassName('slider-single-v3 active-v3').length > 0) {
                setTimeout(() => {
                    if (document.getElementsByClassName('slider-single-v3 active-v3').length > 0) {
                        const height = document.getElementsByClassName('slider-single-v3 active-v3')[0].clientHeight;
                        setHeight(`${height}px`);
                    }
                }, 500);
            }
    ...
    const slideLeft = () => {
        if (slideTotal > 1) {
            ...
            slide.forEach((slid, index) => {
                if (slid.class.includes('proactivede-v3')) {
                    slid.class = 'slider-single-v3 preactivede-v3';
                }
                if (slid.class.includes('proactive-v3')) {
                    slid.class = 'slider-single-v3 proactivede-v3';
                }
            });
            preactiveSlide.class = 'slider-single-v3 preactive-v3';
            activeSlide.class = 'slider-single-v3 active-v3';
            proactiveSlide.class = 'slider-single-v3 proactive-v3';
            setSlides(slide);
            setSlideCurrent(slideCurrentLoc);
            props.onSlideChange(slideCurrentLoc);
    ...
    const sliderClass = (direction) => {
        ...
        if (!props.arrows) {
            sliderClass = 'slider-disabled-v3';
        } else if (props.arrows && !props.arrowBorders) {
            sliderClass = `slider-${direction}-noborders-v3`;
        }
        return sliderClass;
    };
    return [height, slides, nextRef, handlers, slideLeft, sliderClass, slideRight,slideCurrent,goToSlide,slides.length];
}
